...
public class UserDao {
    ...
    public UserModel registUser(Long phone, String name, Long pincode, enumGender gender){
        if(phoneNumberMap.containsKey(phone)){
            UserModel userModel = userHashMap.get(phoneNumberMap.get(phone));
            System.out.println("User already exists with phone number " + phone + "with user id: "+ userModel.getUserId());
            return userModel;
        }
    ...
    public RestaurantModel registerRestaurant(String rastauName, String pincodes, String item,int price,int quantity){
        ...
        //put restaurant data in restarantMap // Restrau Map update kar diya
    ...
    public ReviewModel rateRestaurant(String rastauName, Integer rating, String comment){
        ...
        if(restaurantModel.getListReview() == null || restaurantModel.getListReview().size() == 0){
            ...
            float currentScore = (restaurantModel.getRating() * restaurantModel.getListReview().size() + rating)/restaurantModel.getListReview().size()+1;
    ...
    public List<RestaurantModel> showRestaurant(String sortBy){
        ...
        for(RestaurantModel restaurantModel : r){
            if(restaurantModel.getListPincode().contains(loggedInUser.getPincode()) && restaurantModel.getQuantity() > 0){
    ...
    public List<OrderModel>listOrder(){
        for(OrderModel orderModel : loggedInUser.getListOrder()){
            System.out.println("Order id: "+ orderModel.getOrderId()+" item : "+ orderModel.getItemName()+ " Quantity : "+orderModel.getQuantity()+ " price: "+orderModel.getPrice());
    ...
    class SortByRating implements Comparator<RestaurantModel>{
        ...
        public int compare(RestaurantModel o1, RestaurantModel o2) {
           ...
           if(o1.getRating() == 0.0 || o2.getRating() == 0.0) return 0;
